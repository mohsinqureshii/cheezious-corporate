...
import { prisma, type Prisma } from '@cheezious/database';
...
/**
 * Run the worker loop until `stopWorker` is called.
 *
 * Exported so the same loop can run as its own process or inside the API. It
 * is the identical code either way; only the lifecycle differs.
 */
export async function startWorker(): Promise<void> {
  running = true;
  logger.info({ pollIntervalMs: POLL_INTERVAL_MS, embedded: env.RUN_WORKER }, 'worker started');
...
/**
 * Ask the loop to finish, and wait for the job in flight.
 *
 * Deliberately does not disconnect Prisma or exit the process: when the worker
 * is embedded in the API, the API still needs both. Whoever owns the process
 * owns shutting it down.
 */
