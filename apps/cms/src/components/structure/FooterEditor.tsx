...
export function FooterEditor({ footers, canManage }: { footers: FooterConfig[]; canManage: boolean }) {
  ...
  return (
    <div className="p-06">
      <div className="mb-05 flex items-center gap-04">
        ...
        <select id="footer-locale" className="select w-40" value={locale} onChange={(event) => setLocale(event.target.value)}>
          {footers.map((entry) => (
      ...
      <section className="panel p-06">
        ...
        <div className="mt-05 space-y-05">
          <div>
            ...
            <input id="footer-copyright" className="input" value={copyrightTemplate} disabled={!canManage} onChange={(event) => setCopyright(event.target.value)} />
            <p className="field-helper">
              <code>{'{year}'}</code> is replaced with the current year, so this never goes stale on 1 January.
            </p>
          </div>
          ...
          <div>
            ...
            <input id="footer-region" className="input" value={regionLabel} disabled={!canManage} onChange={(event) => setRegionLabel(event.target.value)} />
          </div>

          <label className="flex items-start gap-03">
            <input type="checkbox" className="checkbox mt-01" checked={showLocaleSwitch} disabled={!canManage} onChange={(event) => setShowLocaleSwitch(event.target.checked)} />
            <span>
              <span className="block text-body-compact text-content-primary">Show the language switch</span>
          ...
          <div>
            ...
            <textarea id="footer-note" rows={2} className="textarea" value={note} disabled={!canManage} onChange={(event) => setNote(event.target.value)} />
          </div>
        </div>
      </section>
      ...
      <section className="panel p-06">
        ...
        {canManage ? (
          <div className="mt-06 flex items-center gap-04">
            <button type="button" onClick={save} disabled={state === 'saving'} className="btn-primary">
      ...
      <ul className="mt-02 space-y-02">
        {items.map((item, index) => (
          <li key={index} className="flex flex-wrap items-end gap-02">
            ...
            {canManage ? (
              <button type="button" className="btn-ghost btn-sm" onClick={() => onChange(items.filter((_, i) => i !== index))}>
                Remove
                <span className="sr-only"> {item[fields[0]!.key] ?? `${title} link ${index + 1}`}</span>
              </button>
            ) : null}
          </li>
        ))}
