...
export async function generateMetadata({
  ...
  return buildRouteSeo({
    ...
    description:
      (job.seoDescription as string) ??
      descriptionParts.join(' ') ??
      `Apply for ${job.title as string} at Cheezious.`,
...
export default async function JobDetailPage({ params }: { params: Promise<RouteParams> }) {
  ...
  const location = job.location as {
    name: string;
    isRemote: boolean;
    city: { name: string; region: { name: string } | null } | null;
  } | null;
  ...
  return (
    <>
      ...
      <article>
          ...
          <div className="container-standard py-section-compact">
            <div className="max-w-4xl">
              ...
              <dl className="mt-8 flex flex-wrap gap-x-10 gap-y-4">
                {location ? (
                  <div>
                    <dt className="text-body-xs uppercase tracking-wider text-ink-faint">
                      Location
                    </dt>
                ...
                {job.postedAt ? (
                  <div>
                    ...
                    <dd className="mt-1 text-body-md text-ink">
                      <time dateTime={job.postedAt as string}>
                        {formatDate(job.postedAt as string, locale)}
                      </time>
                    </dd>
                  </div>
                ) : null}
              ...
              {deadline ? (
                <p
                  className={[
                    ...
                    deadlineSoon
                      ? 'bg-signal-warning/12 text-signal-warning'
                      : 'bg-ink/5 text-ink-soft',
                  ].join(' ')}
                >
                  Applications close on{' '}
                  <time dateTime={deadline}>{formatDate(deadline, locale)}</time>
                </p>
              ) : null}
            ...
            <aside className="lg:col-span-4 lg:col-start-9">
              <div className="sticky top-[calc(var(--header-height)+2rem)] border-t-2 border-brand pt-6">
                ...
                <p className="mt-3 text-body-sm text-ink-soft">
                  Applications are reviewed by our people team. You will receive a reference number
                  when you submit.
                </p>
                <a
                  href="#apply"
        ...
        <section
          id="apply"
          className="border-t border-ink-line bg-paper-sunken scroll-mt-[calc(var(--header-height)+2rem)]"
        >
